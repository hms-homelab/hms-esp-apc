...
// or an operator-supplied slug ("apc_ups_<slug>").
static char device_id[48] = {0};
static char mqtt_base_topic[96] = {0};
static uint8_t device_mac[6] = {0};
static char device_display_name[64] = {0};   // HA device.name; blank => "APC UPS (MAC)"
static char id_slug_override[32] = {0};       // blank => MAC-derived id

// Generate unique device ID from slug override, else from MAC address
static void generate_device_id(void)
{
    esp_efuse_mac_get_default(device_mac);

    if (id_slug_override[0] != '\0') {
        snprintf(device_id, sizeof(device_id), "apc_ups_%s", id_slug_override);
    } else {
...
{
    // Capture identity overrides (empty => MAC-derived id / default display name)
    if (id_slug && id_slug[0])       strlcpy(id_slug_override, id_slug, sizeof(id_slug_override));
    if (device_name && device_name[0]) strlcpy(device_display_name, device_name, sizeof(device_display_name));

    // Generate unique device ID from slug override or MAC address
    generate_device_id();
...
esp_err_t mqtt_publish_discovery(const char *sensor_name, const char *friendly_name, const char *unit, const char *device_class)
{
    ...
    // HA device display name: operator override, else "APC UPS (MAC)"
    char dev_name[80];
    if (device_display_name[0] != '\0') {
        strlcpy(dev_name, device_display_name, sizeof(dev_name));
    } else {
        snprintf(dev_name, sizeof(dev_name), "APC UPS (%02X:%02X:%02X:%02X:%02X:%02X)",
                 device_mac[0], device_mac[1], device_mac[2],
                 device_mac[3], device_mac[4], device_mac[5]);
    }
