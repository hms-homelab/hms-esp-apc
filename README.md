...
> **Powering (important)**: The APC UPS USB port is a **5V sink — it expects VBUS supplied to it and outputs no power**. You cannot power the ESP32-S3 from the UPS. Power the board **externally** (e.g. USB-C 5V + GND), and route that same 5V to the UPS `VCC` pin so the UPS USB interface is powered. There is a single 5V source (your external supply) feeding both the board and the UPS `VCC` — not two sources. Tie all grounds together (external GND, board GND, UPS GND); a common ground is required for USB HID to enumerate.
